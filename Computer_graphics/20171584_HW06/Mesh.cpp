...
void Mesh::init_buffer_objects() 
{
    ...
    glBufferData(GL_ARRAY_BUFFER, mMesh->mNumVertices*3*sizeof(float), &mMesh->mVertices[0], GL_STATIC_DRAW);

    glGenBuffers(1, &normal_buffer);
    glBindBuffer(GL_ARRAY_BUFFER, normal_buffer);
    glBufferData(GL_ARRAY_BUFFER, mMesh->mNumVertices*3*sizeof(float), &mMesh->mNormals[0], GL_STATIC_DRAW);
