...
bool save_scene_to_file(const std::string& filename);
////////////////////////////////////////////////////////////////////////////////
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
  ...
  if (key == GLFW_KEY_C && action == GLFW_PRESS) 
    save_scene_to_file("user_scene.txt");
  if (key == GLFW_KEY_V && action == GLFW_PRESS) 
    init_scene_from_file("user_scene.txt");
}
...
bool init_scene_from_file(const std::string& filename)
{
  ...
  for (int i = 0; i < count; i++)
  {
    ...
    fin >> vt[0] >> vt[1] >> vt[2];
    models[i].set_translate(vt);

    fin >> mr[0][0] >> mr[0][1] >> mr[0][2] >> mr[0][3];
    fin >> mr[1][0] >> mr[1][1] >> mr[1][2] >> mr[1][3];
    fin >> mr[2][0] >> mr[2][1] >> mr[2][2] >> mr[2][3];
    fin >> mr[3][0] >> mr[3][1] >> mr[3][2] >> mr[3][3];
    models[i].set_rotate(mr);

    fin >> vs[0] >> vs[1] >> vs[2];
    models[i].set_scale(vs);
  }
  ...
  // init light
  fin >> g_clear_color[0] >> g_clear_color[1] >> g_clear_color[2];
  fin >> light.pos[0] >> light.pos[1] >> light.pos[2];
...
bool save_scene_to_file(const std::string& filename)
{
  ...
  {
    glm::vec3 m_translate = models[i].translate();
    glm::mat4 m_rotate = models[i].rotate();
    glm::vec3 m_scale = models[i].scale();

    fout << model_names[i] << '\n';

    fout << m_translate[0] << ' ' << m_translate[1] << ' ' << m_translate[2] << '\n';
    fout << m_rotate[0][0] << ' ' << m_rotate[0][1] << ' ' << m_rotate[0][2] << ' ' << m_rotate[0][3] << '\n';
    fout << m_rotate[1][0] << ' ' << m_rotate[1][1] << ' ' << m_rotate[1][2] << ' ' << m_rotate[1][3] << '\n';
    fout << m_rotate[2][0] << ' ' << m_rotate[2][1] << ' ' << m_rotate[2][2] << ' ' << m_rotate[2][3] << '\n';
    fout << m_rotate[3][0] << ' ' << m_rotate[3][1] << ' ' << m_rotate[3][2] << ' ' << m_rotate[3][3] << '\n';
    fout << m_scale[0] << ' ' << m_scale[1] << ' ' << m_scale[2] << '\n';
  }

  fout << cameras.size() << '\n';
  for (int i=0; i<cameras.size(); i++)
  {
    glm::vec3 c_position = cameras[i].position();
    glm::vec3 c_f_dir = cameras[i].front_direction();
    glm::vec3 c_u_dir = cameras[i].up_direction();

    fout << c_position[0] << ' ' << c_position[1] << ' ' << c_position[2] << '\n'; 
    fout << c_f_dir[0] << ' ' << c_f_dir[1] << ' ' << c_f_dir[2] << '\n'; 
    fout << c_u_dir[0] << ' ' << c_u_dir[1] << ' ' << c_u_dir[2] << '\n'; 
  }
...
void compose_imgui_frame()
{
...

  {
    ...
    glm::vec3 position = cameras[cam_select_idx].position();
    if (ImGui::SliderFloat3("tranlsate", glm::value_ptr(position), -2.0f, 2.0f))
      cameras[cam_select_idx].set_position(position);
