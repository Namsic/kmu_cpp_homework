...
void update_buffer_objects()
{
  ...
  if(g_mesh_model == kCube)
  {
    ...
    {
      using namespace triangle_soup;

      g_position_size = sizeof(position);
      g_position_data = position;

      g_color_size = sizeof(color);
      g_color_data = color;

      assert(g_position_size == g_color_size);
      g_num_position = num_position;
    ...
    {
      using namespace vlist_triangles;
      ...
      g_index_size = sizeof(vlist_triangles::index);
      g_index_data = vlist_triangles::index;
